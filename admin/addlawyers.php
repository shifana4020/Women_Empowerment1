...
<body>
        <div class="container">
            <!-- main-heading -->
            <h2 class="main-title-w3layouts mb-2 text-center text-blue">Add lawyers</h2>
            <!--// main-heading -->
...
document.getElementById('msgl4').innerHTML=" ";
    return true;
					 ...
					 <span class="lnr lnr-user"></span>
                      ...
                      <input type="text" class="form-control" placeholder="enter your mail id" name="email" id="email" required onChange="Validata();">
                    </div>
 ...
 function Vallawd()
{
 var val = document.getElementById('district').value;
    if(val==""){
      ...
      <?php 
      include '../connect.php';
      $result1 = mysqli_query($con, "SELECT * from lawyerspecialization");
      while($row1=mysqli_fetch_array($result1)){
      ?>
      <option value="<?php echo $row1['lawyerspeid'];?> "><?php echo $row1['categorylawyer'];?></option>
...
document.getElementById('msgusee').innerHTML=" ";
    return true;
                    ...
                    <input type="submit" name="b"  value="Add lawyer" class="btn btn-primary error-w3l-btn mt-sm-5 mt-3 px-4">
                </form>
            </div>
...
$password=$_POST['password'];
 $s1 = "SELECT * FROM tbl_login WHERE username = '$email'";
	...
	if ($uname!="") 
            ...
            $sq = "INSERT INTO tbl_login (username,password,type1,status) VALUES ('$email','$password','lawyer','1')";
            mysqli_query($con, $sq);
            $sqll = "SELECT * FROM tbl_login WHERE username = '$email' and password = '$password' and type1 = 'lawyer'";
            ...
            if($no > 0)
            {
             ...
             $email=$row['username'];
             $loid=$row['loginid'];
             $sql = "INSERT INTO lawyers (name,age,gender,phonenumber,email,address,court,state,district,idproof,experiencedyears,specialization,loginid,status,file)values('$name',$age,'$gender','$phonenumber','$email','$address','$court','$state','$district','$file','$experiencedyears','$specialization','$loid',1,'$file1')";
